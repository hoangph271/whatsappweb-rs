...
use self::crypto::buffer::{RefReadBuffer, RefWriteBuffer, WriteBuffer};
...
pub(crate) fn calculate_secret_keys(
    secret: &[u8],
    ...
    let secret_key = agreement::agree_ephemeral(
        private_key,
        peer_public_key_alg,
    ...
    hmac::verify(
        &hmac::Key::new(&digest::SHA256, &secret_key_expanded[32..64]),
        &signature,
        &secret[32..64],
    )
    .chain_err(|| "Invalid mac")?;

    let mut buffer = [0u8; 64];

    aes_decrypt(
        &secret_key_expanded[..32],
        &secret_key_expanded[64..],
        &secret[64..144],
        &mut buffer,
    );
...
pub fn verify_and_decrypt_message(
    ...
    let size_without_padding = aes_decrypt(
        enc,
...
pub fn decrypt_media_message(
    ...
    let size_without_padding = aes_decrypt(
        &cipher_key,
        &media_key_expanded[0..16],
        &file_encrypted[..size - 10],
        &mut file,
    );
